...
def _conv_bn(**conv_params):
    ...
    kernel_regularizer = conv_params.setdefault('kernel_regularizer', keras.regularizers.l2(MixNet18.L2_WEIGHT))
    ...
    def f(input_x):
        ...
        bn = keras.layers.BatchNormalization(axis=MixNet18.CHANNEL_AXIS, momentum=MixNet18.BATCH_NORM_DECAY,
                                             gamma_regularizer=keras.regularizers.l2(MixNet18.L2_GAMMA_DECAY),
                                             epsilon=MixNet18.BATCH_NORM_EPSILON)(conv)
        return bn
...
def _shortcut(identity, residual, is_nin=False):
    ...
    stride_width = int(round(identity_shape[MixNet18.ROW_AXIS] / residual_shape[MixNet18.ROW_AXIS]))
    stride_height = int(round(identity_shape[MixNet18.COL_AXIS] / residual_shape[MixNet18.COL_AXIS]))

    if is_nin:
        identity = _conv_bn(filters=residual_shape[MixNet18.CHANNEL_AXIS],
                            kernel_size=(1, 1),
...
def residual_mix_module(input_x, **conv_params):
    """
    一个残差块：
    ...
    if group == 32:
        ...
        group_nums = [int(group / 4)] * 4
    else:
        raise Exception('MixNet中的group数必须是4的倍数！')
        
    kernel_sizes = [kernel_sizes[i] for i, num in enumerate(group_nums) for _ in range(num)]
    
    group_channels = []
    for i in range(group):
        residual = _conv_bn(filters=group_filters, kernel_size=kernel_sizes[i],
                            strides=strides, padding=padding)(input_x)
        residual = _relu(residual)
        group_channels.append(residual)
    residual = keras.layers.concatenate(inputs=group_channels, axis=MixNet18.CHANNEL_AXIS)
    ...
    for i in range(group):
        residual = _conv_bn(filters=group_filters, kernel_size=kernel_sizes[i],
                            strides=(1, 1), padding=padding)(identity)
        residual = _relu(residual)
        group_channels.append(residual)
    residual = keras.layers.concatenate(inputs=group_channels, axis=MixNet18.CHANNEL_AXIS)
    residual = _conv_bn(filters=filters, kernel_size=kernel_size, strides=(1, 1), padding=padding)(residual)
...
class MixNet18(object):
    ...
    def build(input_x):
        """
        构造mixnet18基础网络，接受layers.Input，输出全1000的连接层
        ...
        # 4 * 残差模块
        net = residual_mix_module(net, filters=64, group=16)
        net = residual_mix_module(net, filters=128, strides=(2, 2))
        net = residual_mix_module(net, filters=256, strides=(2, 2))
        net = residual_mix_module(net, filters=384, strides=(2, 2))
